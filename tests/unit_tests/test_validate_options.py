...
class TestValidateOptionsRepeatEverySpecificDate(unittest.IsolatedAsyncioTestCase):
    """Tests for validate_options with the specific-date-of-year repeat type."""

    def _base(self, year_month=3, month_day=15):
        return {
            CONF_REPEAT_MODE: CONF_REPEAT_EVERY,
    ...
    async def test_repeat_every_type_preserved(self):
        result = await validate_options(self._base())
        self.assertEqual(result[CONF_REPEAT_EVERY_TYPE], CONF_REPEAT_EVERY_SPECIFIC_DATE)
    ...
    async def test_month_day_preserved(self):
        ...
        self.assertEqual(result[CONF_REPEAT_MONTH_DAY], 20)

    async def test_year_month_minimum_is_one(self):
        with self.assertRaises(vol.Invalid):
            await validate_options(self._base(year_month=0))

    async def test_year_month_above_twelve_raises(self):
        with self.assertRaises(vol.Invalid):
            await validate_options(self._base(year_month=13))
    ...
    async def test_month_day_below_minimum_raises(self):
        with self.assertRaises(vol.Invalid):
            await validate_options(self._base(month_day=0))

    async def test_month_day_above_maximum_raises(self):
        with self.assertRaises(vol.Invalid):
            await validate_options(self._base(month_day=32))

    async def test_month_day_boundary_values_valid(self):
        for d in (1, 15, 31):
            result = await validate_options(self._base(month_day=d))
